...
def build_payroll_pdf(payroll: sqlite3.Row) -> bytes:
    ...
    pdf.roundRect(40, height - 170, width - 80, 100, 18, stroke=0, fill=1)
    pdf.setFillColorRGB(1, 1, 1)
    pdf.setFont("Helvetica-Bold", 22)
    pdf.drawString(60, height - 105, "Competitive Solutions Co.")
    pdf.setFont("Helvetica", 14)
    pdf.drawString(60, height - 130, "Employee Payroll Slip")

    pdf.setFillColorRGB(0.08, 0.08, 0.08)
    pdf.setFont("Helvetica-Bold", 13)
    pdf.drawString(60, height - 210, f"Employee Name: {payroll['full_name']}")
    pdf.drawString(60, height - 235, f"Month: {payroll['month_label']}")

    rows = [
        ("Base Salary", format_currency(payroll["base_salary"])),
        ("Housing Allowance", format_currency(payroll["housing_allowance"])),
        ("Transport Allowance", format_currency(payroll["transport_allowance"])),
        ("Insurance Deduction", format_currency(payroll["insurance_deduction"])),
        ("Net Salary", format_currency(payroll["net_salary"])),
    ]
    ...
    for index, (label, value) in enumerate(rows):
        ...
        pdf.roundRect(55, y - 20, width - 110, 34, 10, stroke=0, fill=1)
    ...
    pdf.drawString(60, 90, "Generated by the HR system of Competitive Solutions Co.")
    pdf.showPage()
    pdf.save()
